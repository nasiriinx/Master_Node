...
void ui_InforScr5_screen_init(void){
    ...
    lv_chart_set_point_count(ChartTempInfo5, 20);
    lv_chart_set_range(ChartTempInfo5, LV_CHART_AXIS_PRIMARY_Y, -20, 10);
    lv_chart_set_div_line_count(ChartTempInfo5, 10, 10);
    lv_chart_set_zoom_x(ChartTempInfo5, 250);
    lv_chart_set_zoom_y(ChartTempInfo5, 250);
    lv_chart_set_axis_tick(ChartTempInfo5, LV_CHART_AXIS_PRIMARY_X, 2, 1, 10, 10, true, 10);
    lv_chart_set_axis_tick(ChartTempInfo5, LV_CHART_AXIS_PRIMARY_Y, 10, 5, 5, 2, true, 20);
    lv_chart_set_axis_tick(ChartTempInfo5, LV_CHART_AXIS_SECONDARY_Y, 10, 5, 5, 2, false, 25);
    
    lv_chart_series_t * ChartTempInfo5_series_1 = lv_chart_add_series(ChartTempInfo5, lv_color_hex(0x464B55),LV_CHART_AXIS_PRIMARY_Y);

    //receive value                                                             
    static lv_coord_t ChartTempInfo5_series_1_array[] = { -2, -5, -3, -8, -10, -15, 1, 5, 2, -4 };
    lv_chart_set_ext_y_array(ChartTempInfo5, ChartTempInfo5_series_1, ChartTempInfo5_series_1_array);
